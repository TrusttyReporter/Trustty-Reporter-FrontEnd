...
def handle_post_request(main_url, api_key):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            task = call_celery_task(query_text, query_id, temp_dir_name, pdf_files, csv_file)
...
async def convert_csv_encodings(file_paths):
    ...
    for result in results:
        if isinstance(result, Exception):
            return f"An error occurred while converting CSV file encoding: {str(result)}"
...
async def convert_csv_encoding(file_path):
    ...
    if encoding != 'utf-8':
        await convert_csv_to_utf8(file_path, encoding)
